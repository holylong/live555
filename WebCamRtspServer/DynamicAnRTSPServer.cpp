...
#include "DynamicAnRTSPServer.hh"
#include "liveMedia.hh"
#include <string.h>
#include "CameraServerMediaSubsession.hh"

DynamicAnRTSPServer*
DynamicAnRTSPServer::createNew(UsageEnvironment& env, Port ourPort,
			     UserAuthenticationDatabase* authDatabase,
			     unsigned reclamationTestSeconds) {
  int ourSocket = setUpOurSocket(env, ourPort);
  if (ourSocket == -1) return NULL;

  return new DynamicAnRTSPServer(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds);
}

DynamicAnRTSPServer::DynamicAnRTSPServer(UsageEnvironment& env, int ourSocket,
				     Port ourPort,
				     UserAuthenticationDatabase* authDatabase, unsigned reclamationTestSeconds)
  : RTSPServerSupportingHTTPStreaming(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds) {
}

DynamicAnRTSPServer::~DynamicAnRTSPServer() {
}
...
//#define INTERLEAVE_ADUS 1
  ...
  } else if (strcmp(extension, ".webcam") == 0 ){
    printf("webcam\n");
    // ServerMediaSession* sms = ServerMediaSession::createNew(*env, "webcam", "webcam", "Session from webcam");
    NEW_SMS("H.264 Video");
    OutPacketBuffer::maxSize = 100000; // allow for some possibly large H.264 frames
    sms->addSubsession(CameraServerMediaSubsession::createNew(env));
